...
function App() {
  ...
  const handlePhishing = () => {
    alert("⚠️ ALERTA! Phishing detectado. Dados criptografados.");
  ...
  const restartGame = () => {
    ...
    setBudget(200000); 
    setInventory([]); 
    setAnalyzedEvidences([]); 
    setAlertLevel(0); 
    setAssedioLevel(0);
    setTurnCounter(0); 
    setActiveEvent(null); 
    setIsMuralUnlocked(false); 
    setGameStatus("start");
  };
    ...
    <div className="app-layout">
      ...
      <div className="main-body">
        <nav className="nav-menu">
          <button onClick={() => setActiveTab("dashboard")} className={`nav-item ${activeTab === "dashboard" ? "active" : ""}`}>📊 Painel</button>
          <button onClick={() => setActiveTab("interrogatorio")} className={`nav-item ${activeTab === "interrogatorio" ? "active" : ""}`}>🕵️ Interrogatório</button>
          <button onClick={() => setActiveTab("provas")} className={`nav-item ${activeTab === "provas" ? "active" : ""}`}>🗂️ Provas</button>
          <button onClick={() => setActiveTab("email")} className={`nav-item ${activeTab === "email" ? "active" : ""}`}>📧 E-mail</button>
          {isMuralUnlocked && (<button onClick={() => setActiveTab("mural")} className={`nav-item ${activeTab === "mural" ? "active" : ""}`}>📎 Relatório</button>)}
        </nav>
        ...
        <div className="content-area">
          ...
          {activeTab === "email" && (<EmailCenter onPhishingClick={handlePhishing} onDenunciaClick={() => alert("✅ Denúncia Segura! Informação adicionada.")} />)}
